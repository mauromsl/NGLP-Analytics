...
class StoreS3(Store):
    def __init__(self, scope):
        cfg = settings.store_s3_scopes.get(scope)
    ...
    def delete_container(self, container_id):
        ...
        # FIXME: this has a limit of 1000 keys
        delete_info = {
...
class StoreLocal(Store):
    def __init__(self, scope):
        self.dir = settings.store_local_dir
        if self.dir is None:
            raise StoreException("store_local_dir is not defined in config")
        self.buffer_size = settings.store_local_write_buffer_size
...
class TempStore(StoreLocal):
    def __init__(self):
        self.dir = settings.store_tmp_dir
        if self.dir is None:
            raise StoreException("store_tmp_dir is not defined in config")
        self.buffer_size = settings.store_tmp_write_buffer_size
